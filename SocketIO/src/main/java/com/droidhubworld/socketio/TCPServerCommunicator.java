...
public class TCPServerCommunicator {
        ...
        while (mRun) {
            try {
                ...
                if (mConnectListener != null)
                    mConnectListener.connected(socket, socket.getInetAddress(), +socket.getLocalPort(), socket.getLocalSocketAddress(), lastClientIndex, clients.size());
            } catch (IOException e) {
                mRun = false;
                break;
            }
    ...
    public void kickAll(Map<Integer, Client> clients) {
    ...
    public void broadcast(String message, String ignorClient) {
        for (Client client : clients.values()) {
            if (ignorClient != null && client.getSocket().getInetAddress().equals("/" + ignorClient)) {
                continue;
            }
